...
Hyperpowers is available in Kimi Code's plugin marketplace.
...
Go to `Marketplace` > `Hyperpowers` and install it.
...
```text
/plugins install https://github.com/DaRealLebron/hyperpowers/tree/dev
```
...
Kimi Code reads Hyperpowers skills from this repository. There are no copied skills, symlinks, hooks, or extra runtime dependencies.
...
Select Hyperpowers and update it from there. Start a fresh session with `/new` after updating.
...
Kimi Code installs the latest GitHub release for a bare repository URL when one exists. To test unreleased changes before the next Hyperpowers release, install the branch explicitly:
