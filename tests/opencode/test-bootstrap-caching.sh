...
run_present_file_check() {
    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$HYPERPOWERS_PLUGIN_FILE" present
}

run_missing_file_check() {
    mv "$HYPERPOWERS_SKILLS_DIR/using-hyperpowers/SKILL.md" "$TEST_HOME/using-hyperpowers.SKILL.md.bak"

    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$HYPERPOWERS_PLUGIN_FILE" missing
}
